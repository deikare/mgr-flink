...
public abstract class BaseDummyProcessFunction<C extends BaseClassifier> extends KeyedProcessFunction<Long, Example, String> {
    ...
    public BaseDummyProcessFunction(String name, String dataset) {
        this.name = name;
    ...
    public void processElement(Example example, KeyedProcessFunction<Long, Example, String>.Context context, Collector<String> collector) throws Exception {
        ...
        Tuple2<Long, HashMap<String, Long>> trainResult = classifier.train(example);
        Tuple2<String, HashMap<String, Long>> classifyResult = classifier.classify(example, trainResult.f1);
        collector.collect(produceMessage(trainResult.f0, classifyResult, example.getClassName()));
    }
